...
{
	char buf[512] = {0};
	length = min(length, sizeof(buf) - 1);
...
int CHudCaption::Draw(float flTime)
{
	...
	const int distanceBetweenSubs = max(lineHeight / 5, 1);
	...
	for (i=0; i<sub_count; ++i)
	{
		...
		for (j=0; j<sub.lineCount; ++j)
		{
			...
			if (j == 0 && m_hVoiceIcon != 0 && sub.radio)
			{
				...
				SPR_DrawAdditive( 0, xpos-SUB_BORDER_LENGTH-voiceIconWidth-max(voiceIconWidth/8, 1), ypos + lineHeight/2 - voiceIconWidth/2, NULL );
			}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						char numbuf[8];
						strncpy(numbuf, pfile + currentTokenStart, max(tokenLength, sizeof(numbuf)-1));
						numbuf[sizeof(numbuf)-1] = '\0';
